...
public class BookedRoomDAO {
    ...
    public BookedRoom getBookedRoom(int roomId, int quantity, int hotelId, Date arrival, Date department) {
    ...
    public boolean checkSearchChange(ArrayList<BookedRoom> roomcart,int hotelId, Search search){
        for(BookedRoom br:roomcart){
            if(!checkQuanRoom(br.getId(), br.getQuantity(), hotelId, search.getArrival(), search.getDepartment())) return false;
        }
        return true;
    }
}
